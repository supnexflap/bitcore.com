...
if (submitButton) {
  submitButton.addEventListener('click', (e) => {
    ...
    if (!phoneRegex.test(phone)) {
      showSuccessModal("Введите корректный номер телефона в формате +7 (777) 123-45-67");
      return;
    }
    ...
    .then(data => {
      if (data.ok) {
        ...
        showSuccessModal("Ошибка при отправке!");
        console.error(data);
      }
    })
    .catch(err => {
      showSuccessModal("Ошибка соединения с Telegram");
      console.error(err);
    });
  });
}
